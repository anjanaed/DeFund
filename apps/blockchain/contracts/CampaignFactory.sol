...
contract CampaignFactory is AccessControl, ReentrancyGuard, Pausable {
    ...
    mapping(uint256 => mapping(address => uint256)) public campaignContributions; // campaignId => contributor => amount
    ...
    event CampaignFlagged(uint256 indexed campaignId, address indexed admin, string reason);
    ...
    modifier onlyContributor(uint256 _campaignId) {
        require(campaignContributions[_campaignId][msg.sender] > 0, "Not a contributor");
        _;
    }
    ...
    modifier validMilestone(uint256 _milestoneId) {
        require(_milestoneId > 0 && _milestoneId <= milestoneCounter, "Invalid milestone");
        _;
    }
    ...
    function createCampaign(
        ...
        for (uint256 i = 0; i < _milestones.length; i++) {
            require(_milestones[i].amountRequired > 0, "Milestone amount must be positive");
            require(_milestones[i].deadline <= _deadline, "Milestone deadline exceeds campaign deadline");
            totalMilestoneAmount += _milestones[i].amountRequired;
        }
        require(totalMilestoneAmount == _fundGoal, "Milestone amounts must equal fund goal");
     ...
     */
    function flagCampaign(uint256 _campaignId, string memory _reason) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE)
        validCampaign(_campaignId)
    {
        ...
        validCampaign(_campaignId)
    {
        ...
        require(
            campaign.creator == msg.sender || hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "Not authorized"
        );
    ...
    {
        Campaign storage campaign = campaigns[_campaignId];
        require(campaign.paymentToken == PaymentToken.ETH, "Campaign accepts USDC only");
        ...
        if (campaign.raisedAmount >= campaign.fundGoal && campaign.status == CampaignStatus.Active) {
     ...
     */
    function contributeUSDC(uint256 _campaignId, uint256 _amount) 
        external 
        nonReentrant
        whenNotPaused
        validCampaign(_campaignId)
    ...
    {
        Campaign storage campaign = campaigns[_campaignId];
        require(campaign.paymentToken == PaymentToken.USDC, "Campaign accepts ETH only");
     ...
     */
    function submitMilestoneForVoting(uint256 _milestoneId, string memory _proofIpfsHash) 
        external 
        ...
        for (uint256 i = 0; i < campaignMilestones.length; i++) {
            if (campaignMilestones[i] == _milestoneId) {
                ...
                if (i > 0) {
                    ...
                    require(
                        milestones[previousMilestoneId].status == MilestoneStatus.Completed,
        ...
        onlyContributor(milestones[_milestoneId].campaignId)
    {
        ...
        require(milestone.status == MilestoneStatus.Voting, "Milestone not open for voting");
        ...
        uint256 voteWeight = campaignContributions[milestone.campaignId][msg.sender];
     ...
     * @param _milestoneId Milestone ID
     */
    function finalizeMilestoneVoting(uint256 _milestoneId) 
        external 
        validMilestone(_milestoneId)
    {
        Milestone storage milestone = milestones[_milestoneId];
        ...
        require(milestone.status == MilestoneStatus.Voting, "Milestone not in voting");
        require(block.timestamp >= milestone.votingEndTime, "Voting period not ended");
        
        uint256 totalVotes = milestone.votesFor + milestone.votesAgainst;
        uint256 quorumRequired = (campaign.raisedAmount * MIN_QUORUM_PERCENTAGE) / 100;
        
        // Check quorum
        bool quorumMet = totalVotes >= quorumRequired;
        
        // Simple majority of votes cast (weighted by contribution)
        bool approved = quorumMet && (milestone.votesFor > milestone.votesAgainst);
        ...
        onlyCampaignCreator(milestones[_milestoneId].campaignId)
    {
        ...
        require(
            campaign.withdrawnAmount + milestone.amountRequired <= campaign.raisedAmount,
            "Insufficient funds"
        );
        ...
        for (uint256 i = 0; i < campaign.milestoneIds.length; i++) {
            if (milestones[campaign.milestoneIds[i]].status != MilestoneStatus.Completed) {
                allCompleted = false;
                break;
            }
        }
        ...
        if (campaign.paymentToken == PaymentToken.ETH) {
            (bool success, ) = payable(campaign.creator).call{value: milestone.amountRequired}("");
    ...
    // ============ Multi-Sig Refund Functions ============
    
    /**
     * @notice First admin proposes refund for campaign
     * @param _campaignId Campaign ID
     */
    function proposeRefund(uint256 _campaignId) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE)
        validCampaign(_campaignId)
    {
        ...
        require(campaignRefundProposal[_campaignId] == 0, "Refund already proposed");
        require(campaign.raisedAmount > campaign.withdrawnAmount, "No funds to refund");
     ...
     * @param _campaignId Campaign ID
     */
    function approveRefund(uint256 _campaignId) 
        external 
        onlyRole(DEFAULT_ADMIN_ROLE)
        validCampaign(_campaignId)
    {
        ...
        onlyContributor(_campaignId)
    {
        ...
        uint256 contributorAmount = campaignContributions[_campaignId][msg.sender];
     ...
     * @param _campaignId Campaign ID
     */
    function getRefundProposal(uint256 _campaignId) 
        external 
        view 
        validCampaign(_campaignId)
        returns (RefundProposal memory) 
    {
        uint256 proposalId = campaignRefundProposal[_campaignId];
        require(proposalId > 0, "No refund proposal");
     ...
     */
    function grantCreatorRole(address _account) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function revokeCreatorRole(address _account) external onlyRole(DEFAULT_ADMIN_ROLE) {
